...
#proxies = {'https': 'http://172.105.43.37:3535/'}
	...
	if os.path.isfile(tokenfile):
		...
		if os.path.isfile(confi):
			with open("config.json") as json_data:
				...
				data = {"email": email, "password": password,"aid":"91955485578","lotame_cookie_id":"","guest_token":"RK3M4hUEPNPzFTr1s2CJ000000000000","platform":"app","version":"2.51.37"}
				token = requests.post('https://whapi.zee5.com/v1/user/loginemail_v2.php', data=json.dumps(data), headers=headers).json()['access_token']
				with open(tokenfile, 'w') as f:
					f.write(json.dumps(token, indent=4))

def session():
	session_token = requests.get("https://useraction.zee5.com/token/platform_tokens.php?platform_name=androidtv_app").json()["token"]
...
def single(id):
	...
	params = {
	'content_id': id,
	'device_id': 'RK3M4hUEPNPzFTr1s2CJ000000000000',
	...
	'uid': 'f0713d0e-257d-47fe-b0de-f27d7fbff9c5',
	'ppid': 'RK3M4hUEPNPzFTr1s2CJ0000000000000',
	'version': 12
	}
	resp = requests.post(PLAYBACK_URL, headers=headers, params=params, json=data).json()
...
def getseries(seriesID):
	...
	if not args.season:
		SeasonNo = str(input("\nEnter Season Number: "))
	res = requests.get(api+seriesID, params=series_params, headers={'x-access-token': session()}).json()
	...
	for num in itertools.count(1):
		...
		res = requests.get(api, params=season_params, headers=session()).json()
...
def parsempd(url):
	...
	videoslist = []
	subtitlelist = []
	mpd = requests.get(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36'}).text
	if mpd:
		mpd = re.sub(r"<!--  -->","",mpd)
...
def ismdash(streamid, filename, mpdurl, baseurl):
	...
	mpd = requests.get(url)
	if 'prime' in mpdurl:
		subprocess.run([ytdlexe, '--allow-unplayable-formats', '-k', '--quiet', '--no-warnings', '--user-agent', 'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/83.0.4103.61 Mobile/15E148 Safari/604.1', '-f', streamid, '--fixup', 'never', '--proxy', 'http://172.105.43.37:3535/', mpdurl, '-o', filename, '--external-downloader',aria2cexe, '--external-downloader-args', '-x 16 -s 16 -k 1M'])
		time.sleep(1)
...
def do_decrypt(pssh, drmdata, nl):
	...
	resp = requests.post('https://spapi.zee5.com/widevine/getLicense', data=chal, headers=headers)
	lic = resp.content
